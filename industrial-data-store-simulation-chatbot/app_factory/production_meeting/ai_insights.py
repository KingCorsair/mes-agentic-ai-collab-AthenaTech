    ...
    try:
        with st.spinner("Generating AI insights using production meeting agents..."):
            ...
            if response.get('success', False):
                ...
                # Light attribution so users know this is AI-generated, without exposing internals.
                output += "\n\n<small><i>AI-generated insight</i></small>"
    ...
    with col1:
        ...
        if generate_button:
            if use_cached == "Cached (Fast)":
                ...
                if cached_analysis:
                    ...
                    st.info("No saved briefing yet. Switch **Analysis Mode** to *Live Agent* above, or use **Generate today's briefing** on the dashboard.")
        ...
        if hasattr(st.session_state, 'current_analysis') and use_cached == "Live Agent (Comprehensive)":
            ...
            if (use_cached == "Cached (Fast)" and 
                ...
                # Point users to the in-app briefing generator if nothing is cached.
                if not cache_status['is_fresh']:
                    st.info("💡 **Tip**: Use **Generate today's briefing** on the dashboard to enable fast cached insights.")
