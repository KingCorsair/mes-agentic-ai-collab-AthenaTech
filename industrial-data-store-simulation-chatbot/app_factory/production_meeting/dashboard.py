...
from .daily_analysis_scheduler import generate_briefing_now
...
def format_freshness(generated_at: str) -> str:
    """Turn an ISO timestamp into a friendly 'as of' string for business users."""
...
def run_briefing_generation(generate_data: bool = False):
    """Generate today's briefing from the UI, then refresh the page.

    Shows a spinner because the analysis takes roughly one to two minutes.
    """
    try:
        with st.spinner("Generating today's briefing… this takes about a minute."):
            generate_briefing_now(generate_data=generate_data)
        st.cache_data.clear()
        st.success("Briefing updated.")
        st.rerun()
    except Exception as e:
        st.error(
            "We couldn't generate the briefing just now. "
            "Please try again in a moment, or contact your administrator if it keeps happening."
        )
...
def display_ai_summary_card():
    ...
    with st.expander("AI Daily Briefing", expanded=True):
        # Freshness + refresh action row, so users can trust and update the briefing.
        head_col, action_col = st.columns([3, 1])
        with head_col:
            generated_at = cached.get('generated_at', 'Unknown') if cached else 'Unknown'
            st.caption(f"Data as of {format_freshness(generated_at)}")
        with action_col:
            if st.button("↻ Generate today's briefing", use_container_width=True,
                         key="generate_briefing_card"):
            ...
            if cached:
                ...
                if 'executive_summary' in analyses:
                    ...
                    if raw_summary:
                        ...
                        clean_summary = remove_status_emojis(exec_summary or raw_summary)
                        st.markdown(clean_summary)
                    else:
                        st.info("No briefing yet today. Use **Generate today's briefing** above to create one.")
                else:
                    st.info("No briefing yet today. Use **Generate today's briefing** above to create one.")
            else:
                st.info("No briefing yet today. Use **Generate today's briefing** above to create one.")

            # Let users take the briefing away for emails and slide decks.
            if clean_summary:
                st.download_button(
                    "⤓ Download briefing",
                    data=clean_summary,
                    file_name=f"daily_briefing_{datetime.now().strftime('%Y-%m-%d')}.txt",
                    mime="text/plain",
...
def run_production_meeting():
    ...
    with control_cols[1]:
        # Data freshness, so users know how current the numbers are.
        latest = cache_manager.get_latest_analysis(max_age_hours=48)
        generated_at = latest.get('generated_at', 'Unknown') if latest else 'Unknown'
        st.write(f"🕒 {datetime.now().strftime('%H:%M')}")
        st.caption(f"Briefing as of {format_freshness(generated_at)}")
